# Activations script for add the lqtk module to the lua path when
# building lqtk using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if lqtk is installed, e.g. via luarocks.
...
lualqtk_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$lualqtk_dir/src/activate.sh" -o ! -e "$lualqtk_dir/src/main.cpp" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "src" directory ***'

else

    echo "Setting lua paths for: $lualqtk_dir"

    add_lua_path="$lualqtk_dir/src/?.lua:$lualqtk_dir/src/?/init.lua"
    add_lua_cpath="$lualqtk_dir/src/build"
...
unset lua_cmd this_dir lualqtk_dir add_lua_path add_lua_cpath lua_version lua_path_vers vers default_version \
